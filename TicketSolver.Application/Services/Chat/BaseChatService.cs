...
public class BaseChatService<TTickets>(
    IChatRepository chatRepository,
    ITicketsRepository<TTickets> ticketRepository,
    ILogger<BaseChatService<TTickets>> logger,
    ...
    where TTickets : Tickets
{
    ...
    public async Task<bool> IsTicketAssignedToTechnicianAsync(int ticketId,
        CancellationToken cancellationToken = default)
    {
        try
        {
    ...
    public async Task TransferToTechnicianAsync(int ticketId, string userId,
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (!availableTechnicians.Any())
            {
                ...
                await SendSystemMessageAsync(new SendSystemMessageRequestDto
                {
                    TicketId = ticketId,
                    Text =
                        "Todos os técnicos estão ocupados no momento. Seu ticket foi adicionado à fila de atendimento.",
                    MessageType = "text"
                }, cancellationToken);
            ...
            await SendSystemMessageAsync(new SendSystemMessageRequestDto
            {
                TicketId = ticketId,
                Text =
                    "Seu ticket foi transferido para um técnico especializado. Em breve você receberá uma resposta personalizada.",
                MessageType = "text"
            }, cancellationToken);
    ...
    public async Task<IEnumerable<TicketPendingResponseDto>> GetTicketsPendingTechnicianResponseAsync(
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            foreach (var ticket in ticketsEnumerable)
            {
                try
                {
                    ...
                    logger.LogInformation(
                        $"Chat encontrado para ticket {ticket.Id} - TotalMessages: {chat.TotalMessages}");
                    ...
                    pendingTickets.Add(new TicketPendingResponseDto
                    {
                        ...
                        LastUserMessage =
                            lastUserMessage.text?.Substring(0, Math.Min(100, lastUserMessage.text.Length)) ?? "",
    ...
    )
    {
        try
        {
            ...
            if (userMessageCount == 1)
            {
                ...
                var systemPrompt =
                    $"Você é um assistente técnico que precisa me ajudar a resolver o seguinte problema: {ticket?.Title}, {ticket?.Description}. Mantenha o contexto da conversa e referencie mensagens anteriores quando relevante.";
            ...
            logger.LogInformation(
                $"Histórico preparado com {groqHistory.Count} mensagens para ticket {request.TicketId}");
    ...
    public async Task<ChatHistoryResponseDto> GetChatHistoryAsync(int ticketId,
        CancellationToken cancellationToken = default)
    {
        return await GetChatHistoryAsync(ticketId, 1, int.MaxValue, cancellationToken);
    }

    public async Task<ChatHistoryResponseDto> GetChatHistoryAsync(int ticketId, int page, int pageSize,
        CancellationToken cancellationToken = default)
    {
        try
        {
    ...
    public async Task MarkMessagesAsReadAsync(MarkAsReadRequestDto request,
        CancellationToken cancellationToken = default)
    {
        try
        {
            await chatRepository.MarkMessagesAsReadAsync(request.TicketId, request.UserId, request.UserType,
                cancellationToken);
            logger.LogInformation("Mensagens marcadas como lidas para o usuário {UserId} no ticket {TicketId}",
                request.UserId, request.TicketId);
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Erro ao marcar mensagens como lidas para o usuário {UserId} no ticket {TicketId}",
                request.UserId, request.TicketId);
            throw;
        }
    }

    public async Task<IEnumerable<ChatSummaryDto>> GetChatsWithUnreadMessagesAsync(string userId, string userType,
        CancellationToken cancellationToken = default)
    {
        try
        {
    ...
    public async Task<IEnumerable<ChatSummaryDto>> GetRecentChatsAsync(int limit = 10,
        CancellationToken cancellationToken = default)
    {
        try
        {
    ...
    public async Task<ChatStatisticsDto> GetChatStatisticsAsync(int ticketId,
        CancellationToken cancellationToken = default)
    {
        try
        {
    ...
    public async Task<ChatResponseDto> StartChatAsync(int ticketId, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (existingChat == null)
            {
                ...
                logger.LogInformation(
                    "Chat será criado quando a primeira mensagem for enviada para o ticket {TicketId}", ticketId);
    ...
    public async Task<bool> CanAccessChatAsync(int ticketId, string userId, string userType,
        CancellationToken cancellationToken = default)
    {
        try
        {
    ...
    public async Task<IEnumerable<ChatMessageDto>> SearchMessagesAsync(ChatSearchRequestDto request,
        CancellationToken cancellationToken = default)
    {
        try
        {
    ...
    public async Task<int> GetUnreadMessageCountAsync(int ticketId, string userId, string userType,
        CancellationToken cancellationToken = default)
    {
        try
        {
    ...
    public async Task<IEnumerable<ChatParticipantDto>> GetChatParticipantsAsync(int ticketId,
        CancellationToken cancellationToken = default)
    {
        try
        {
    ...
    public async Task<ChatMessageResponseDto> SendSystemMessageAsync(SendSystemMessageRequestDto request,
        CancellationToken cancellationToken = default)
    {
        try
        {
    ...
    public async Task<ChatActivitySummaryDto> GetChatActivitySummaryAsync(DateTime startDate, DateTime endDate,
        CancellationToken cancellationToken = default)
    {
        try
        {
    ...
    public async Task ArchiveChatAsync(int ticketId, bool isArchived, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            logger.LogInformation("Chat do ticket {TicketId} {Action}", ticketId,
                isArchived ? "arquivado" : "desarquivado");
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Erro ao {Action} chat do ticket {TicketId}", isArchived ? "arquivar" : "desarquivar",
                ticketId);
            throw;
        }
    }
