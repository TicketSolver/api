...
public class UsersController(IUsersService usersService) : ShellController
{
    ...
    public async Task<ActionResult<Users>> GetUser(string id, CancellationToken cancellationToken)
    {
        ...
        {
            throw new TicketSolver.Application.Exceptions.Http.NotFoundException("Usuário não encontrado");
        }
    }
    ...
    public async Task<IActionResult> DeleteUser(string id, CancellationToken cancellationToken)
    {
        if (AuthenticatedUser.UserId != id && AuthenticatedUser.DefUserType != eDefUserTypes.Admin)
            throw new TicketSolver.Application.Exceptions.Http.ForbiddenException("Você não possui permissão para editar este usuário!");
        if(AuthenticatedUser.UserId == id && AuthenticatedUser.DefUserType == eDefUserTypes.Admin)
            throw new TicketSolver.Application.Exceptions.Http.ForbiddenException("Você não pode deletar o seu próprio usuário!");
        try
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (AuthenticatedUser.DefUserType != eDefUserTypes.Admin)
            throw new TicketSolver.Application.Exceptions.Http.ForbiddenException("Você não possui permissão para acessar usuários do Tenants!");
