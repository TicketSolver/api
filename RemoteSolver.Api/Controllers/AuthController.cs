...
) : Controller
{
    ...
    public async Task<IActionResult> Register([FromBody] RegisterModel model, CancellationToken cancellationToken)
    {
        ...
        {
            throw new TicketSolver.Application.Exceptions.Http.BadRequestException("Usuário já registrado!");
        }
        catch (InvalidPublicKeyException)
        {
            throw new TicketSolver.Application.Exceptions.Http.UnauthorizedException("Chave pública inválida!");
        }
        catch (UserNotFoundException)
        {
            throw new TicketSolver.Application.Exceptions.Http.ForbiddenException("Chave pública inválida!");
        }
    }
